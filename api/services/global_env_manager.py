...
class GlobalEnvManager:
    def _validate_and_adjust_dates(self, start: str, end: str, interval: str) -> Tuple[str, str]:
    ...
    def _fetch_data(self, pair, interval, start, end):
        adjusted_start, adjusted_end = self._validate_and_adjust_dates(start, end, interval)
    ...
    def create_env(self, pair, interval, initial_balance, start, end):
        data, adjusted_start, adjusted_end = self._fetch_data(pair, interval, start, end)
    ...
    def save_env(self, pair, interval, env_data_serialized):
        try:
            ...
            if env:
                ...
                env = GlobalEnvironment(pair=pair, interval=interval, data=env_data_serialized)
                db.session.add(env)
            db.session.commit()
        except Exception:
            db.session.rollback()
    ...
    def update_env_data(self, pair, interval):
        ...
        if adjusted_end <= now:
            ...
            env_data["adjusted_end"] = now.strftime("%Y-%m-%d")
            self.save_env(env_data["pair"], env_data["interval"], cloudpickle.dumps(env_data))
